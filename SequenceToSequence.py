...
class Seq2Seq(object):
    ...
    def input_feed(self):
        encoder_inputs = tf.placeholder(tf.int32, shape=[self.hidden_size, None], name='encoder_inputs')
        encoder_inputs_length = tf.placeholder(tf.int32, shape=[self.hidden_size, ], name='encoder_inputs_length')
        keep_prob = tf.placeholder(tf.float32, shape=(), name='keep_prob')
        decoder_inputs = tf.placeholder(tf.int32, shape=[self.hidden_size, None], name='decoder_inputs')
        decoder_inputs_length = tf.placeholder(tf.int32, shape=[self.hidden_size, ],
                                               name='decoder_inputs_length')
    ...
    def export(self, sess, save_path='model/saved_model/'):
        tf.estimator.Estimator.export_savedmodel(export_dir_base=save_path, serving_input_receiver_fn=self.input_feed(),
